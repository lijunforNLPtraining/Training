...
def svm_model(X_train, X_test, y_train, y_test,model_):
    train_data = model_[X_train]
    test_data = model_[X_test]
    clf = svm.SVC(C=10, kernel='rbf', gamma=10, decision_function_shape='ovr')
    pca = decomposition.PCA(n_components=500)
    train_data = pca.fit_transform(train_data)
    test_data = pca.fit_transform(test_data)
    clf_res = clf.fit(train_data, y_train)
    y_pred = clf_res.predict(test_data)
...
if __name__ == '__main__':
    ...
    svm_model(X_train, X_test, y_train, y_test,newword2vector)
