...
LTP_DATA_DIR = 'D:\\Eclipse_workplace\\Training\\NLPTraining\\project01\\model\\ltp-models'
...
####################################################
    #下面开始写第二种获取言论内容的方法
    # 获取中心词之后的第一个符号（，。！）的索引
    def get_first_wp_after_keyword(self, postags, key_word_index):
        for i in range(key_word_index + 1, len(postags)):
            if postags[i] == 'wp':
    ...
    def keyword_senteces(self,word_list,postags,key_word_index):
        sentences = []
        first_wp_index = self.get_first_wp_after_keyword(postags,key_word_index)
        first_sentence = ''.join(word_list[key_word_index:first_wp_index])#得到关键词后面的第一个句子
        ...
        if len(other_related_sentence_wp_index_list) > 0:#如果找到了
            for i ,index in enumerate(other_related_sentence_wp_index_list):#一个个的截取
                if i ==0:
                    sentences.append(word_list[first_wp_index:index])#当为第一个句子的时候，把它的索引和第一个句子结尾的索引截取就得到了第一个相关句子
                else:
                    sentences.append(word_list[other_related_sentence_wp_index_list[i-1]:index])#第二个句子就是当前索引与上一个索引截取就对了
        return sentences
